...
| 화면 | DCS 개요(전체 화면) · 초안(근무 목록 → 전 항목 표시·선택·코멘트·승인 / 확정 일지) |
...
> 알람에 안 걸리는 신호는 전혀 못 잡습니다. 시나리오 20종 중 알람이 울리는 건 **7종**
> (6·10·16~20)이라 임시 엔진의 사정권은 거기까지이고, 알람 미발생 **13종**이 `engine/` 이
> 존재하는 이유입니다. (설계값이며 실측 포함률은 정답지 도착 후 측정합니다 — #5)
